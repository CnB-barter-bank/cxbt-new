...
const initializeWeb3Modal = () => {
  ...
  if (typeof window === 'undefined') {
    ...
    web3Modal = {
      open: () => {
        console.error(
          '[Web3Modal] Невозможно открыть модальное окно - Web3Modal не инициализирован (SSR)'
        )
  ...
  try {
    ...
    web3Modal = {
      open: () => {
        console.error(
          '[Web3Modal] Невозможно открыть модальное окно - Web3Modal не инициализирован'
        )
        throw new Error('Web3Modal не инициализирован. Проверьте консоль для деталей.')
      },
      close: () => {
...
console.error = function (...args) {
  try {
    ...
    if (errorStr.includes('getChainId') && errorStr.includes('not a function')) {
      console.error(
        '[GLOBAL ERROR HANDLER] Обнаружена ошибка "getChainId is not a function":',
        args
      )
...
window.addEventListener('error', event => {
  if (
    event.message &&
    event.message.includes('getChainId') &&
...
window.addEventListener('unhandledrejection', event => {
  if (
    ...
    event.reason.message.includes('getChainId') &&
    event.reason.message.includes('not a function')
  ) {
    console.error(
      '[GLOBAL UNHANDLED REJECTION] Обнаружена ошибка "getChainId is not a function":',
      event.reason
    )
    console.error(
      '[GLOBAL UNHANDLED REJECTION] Stack trace:',
...
const trackedGetConnection = async config => {
  ...
  try {
    const result = await getConnection(config)
    ...
    console.log(
      `[TRACKED getConnection] Вызов #${callId} - Тип connector:`,
      typeof result?.connector
    )

    if (result?.connector) {
      console.log(
        `[TRACKED getConnection] Вызов #${callId} - Методы connector:`,
        Object.getOwnPropertyNames(Object.getPrototypeOf(result.connector))
      )
      console.log(
        `[TRACKED getConnection] Вызов #${callId} - Есть ли getChainId?`,
        typeof result.connector.getChainId
      )
    }
...
export function useWalletConnect() {
  ...
  const updateWalletState = async () => {
    try {
      ...
      console.log(
        '[useWalletConnect] updateWalletState - connection.connector:',
        connection?.connector
      )
      console.log(
        '[useWalletConnect] updateWalletState - Тип connector:',
        typeof connection?.connector
      )

      // Проверяем наличие методов на connector
      if (connection?.connector) {
        console.log(
          '[useWalletConnect] updateWalletState - Методы connector:',
          Object.getOwnPropertyNames(Object.getPrototypeOf(connection.connector))
        )
        console.log(
          '[useWalletConnect] updateWalletState - Есть ли getChainId?',
          typeof connection.connector.getChainId
        )
        ...
        console.log(
          '[useWalletConnect] updateWalletState - connection.chainId:',
          connection.chainId
        )
      }
      ...
      console.log(
        '[useWalletConnect] updateWalletState - address:',
        address.value,
        'isConnected:',
        isConnected.value,
        'chainId:',
  ...
  const ensureBaseNetwork = async () => {
    try {
      ...
      console.log(
        '[WalletConnect] ensureBaseNetwork - connection.connector:',
        connection?.connector
      )
      ...
      if (connection?.connector) {
        console.log(
          '[WalletConnect] ensureBaseNetwork - Методы connector:',
          Object.getOwnPropertyNames(Object.getPrototypeOf(connection.connector))
        )
        console.log(
          '[WalletConnect] ensureBaseNetwork - Есть ли getChainId?',
          typeof connection.connector.getChainId
        )
      ...
      if (currentChainId !== BASE_CHAIN_ID) {
        console.warn(
          `[WalletConnect] Неподдерживаемая сеть: ${currentChainId}. Требуется Base (${BASE_CHAIN_ID})`
        )
  ...
  const autoReconnect = async () => {
    ...
    console.log(
      '[WalletConnect] autoReconnect - isConnected.value ДО reconnect:',
      isConnected.value
    )
    console.log(
      '[WalletConnect] autoReconnect - walletStore.isConnected ДО reconnect:',
    ...
    try {
      ...
      console.log(
        '[WalletConnect] autoReconnect - isConnected.value ПОСЛЕ reconnect:',
        isConnected.value
      )
      console.log(
        '[WalletConnect] autoReconnect - walletStore.isConnected ПОСЛЕ reconnect:',
        walletStore.isConnected
      )
  ...
  const unwatchAccount = watchAccount(wagmiConfig, {
    onChange(account) {
      ...
      if (
        ...
        console.log(
          '[useWalletConnect] watchAccount onChange - Account не определён или все поля undefined, сбрасываем состояние'
        )
        ...
        console.log(
          '[useWalletConnect]   state ПОСЛЕ сброса - address:',
          address.value,
          'isConnected:',
          isConnected.value
        )
      ...
      if (account?.connector) {
        console.log(
          '[useWalletConnect] watchAccount onChange - Методы connector:',
          Object.getOwnPropertyNames(Object.getPrototypeOf(account.connector))
        )
        console.log(
          '[useWalletConnect] watchAccount onChange - Есть ли getChainId?',
          typeof account.connector.getChainId
        )
      ...
      if (isConnected.value && account.chainId) {
        try {
          ...
          walletProvider.value = publicClient
          console.log(
            '[useWalletConnect] watchAccount onChange - publicClient получен:',
            publicClient
          )
        } catch (error) {
          console.error(
            '[useWalletConnect] Ошибка при получении public client в watchAccount:',
            error
          )
        }
      } else {
  ...
  watch(
    ...
    (newAddress, oldAddress) => {
      console.log(
        '[useWalletConnect] watch address triggered - old:',
        oldAddress,
        'new:',
        newAddress
      )
  ...
  onMounted(async () => {
    ...
    console.log(
      '[WalletConnect] onMounted - isConnected.value ДО autoReconnect:',
      isConnected.value
    )
    ...
    console.log(
      '[WalletConnect] onMounted - localStorage wallet data:',
      localStorage.getItem('cxbt-wallet')
    )

    await autoReconnect()

    console.log(
      '[WalletConnect] onMounted - isConnected.value ПОСЛЕ autoReconnect:',
      isConnected.value
    )
    console.log(
      '[WalletConnect] onMounted - walletStore.isConnected ПОСЛЕ autoReconnect:',
      walletStore.isConnected
    )
    ...
    console.log(
      '[WalletConnect] onMounted - walletStore.isConnected ПОСЛЕ задержки:',
      walletStore.isConnected
    )
    ...
    if (shouldAutoOpen && !hasAutoOpened.value && !isConnected.value && !walletStore.isConnected) {
      ...
      console.log('[WalletConnect] onMounted - !walletStore.isConnected:', !walletStore.isConnected)
      console.log(
        '[WalletConnect] onMounted - ПРОВЕРКА: Все условия ИСТИННЫ, открываем модальное окно'
      )
      ...
      console.log('[WalletConnect] onMounted - !walletStore.isConnected:', !walletStore.isConnected)
      console.log(
        '[WalletConnect] onMounted - ПРОВЕРКА: Условие ЛОЖНО, модальное окно НЕ открывается'
      )
    }
  })
