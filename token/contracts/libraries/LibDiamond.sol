...
library LibDiamond {
    ...
    function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        require(_functionSelectors.length != 0, "LibDiamondCut: No selectors in facet to cut");
        DiamondStorage storage ds = diamondStorage();
        // Проверяем, что все селекторы существуют и принадлежат другому адресу
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            bytes4 selector = _functionSelectors[selectorIndex];
            address oldFacetAddress = ds.selectorToFacetAndPosition[selector].facetAddress;
            require(oldFacetAddress != address(0), "LibDiamondCut: Function does not exist");
            require(oldFacetAddress != _facetAddress, "LibDiamondCut: Can't replace function with same function");
        }
        // Если новый адрес facet еще не существует, добавляем его
        if (!isFacet(ds, _facetAddress)) {
            ds.facetFunctionSelectors[_facetAddress].facetAddressPosition = ds.facetAddresses.length;
            ds.facetAddresses.push(_facetAddress);
        }
